...
<script type="text/javascript">
	...
	function toggleRelyMessage(messageId){
		...
		$("#relyeMessageBlock_"+messageId).slideDown(1500);
	}
		...
		<c:if test="${not empty at.atMeMessageList}">
			...
			<c:forEach items="${at.atMeMessageList}" var="message">
				<div class="messageContent">
					...
					<fmt:parseDate value="${message.createTime}" type="date"
						var="createTime" pattern="yyyyMMdd HH:mm:ss" />
					<div class="messageCreateTime">
				...
				<c:if test="${not empty message.relyMessageList}">
					<div class="toggleRelyMessage"
						onclick="toggleRelyMessage(${message.mid})"
						id="toggleRelyMessage_${message.mid}">display</div>
					<div class="relyeMessageBlock"
						id="relyeMessageBlock_${message.mid}">
						<c:forEach items="${message.relyMessageList}" var="relyMessage">
							<div class="relyMessage">
								<img class="userImg" id="headImg_${relyMessage.uid}"
									align="left"
								...
								<fmt:parseDate value="${relyMessage.relyTime}" type="date"
									var="relyTime" pattern="yyyyMMdd HH:mm:ss" />
								<div class="relyMessageTime">
									<fmt:formatDate value="${relyTime}"
										pattern="yyyy年MM月dd日  HH点mm分ss秒" />
								</div>
