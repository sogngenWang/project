...
<script type="text/javascript">
	...
	function displayStore(areaid){
		...
		$.ajax({
			type : "post",//使用post方法访问后台  
			dataType : "json",//返回json格式的数据  
			url : "./listStore?areaid="+areaid,//要访问的后台地址  
			contentType : "application/json;charset=utf-8",
			data : "", //要发送的数据  
			cache : 'false', 
			beforeSend : function() { //数据发送前报告
	...
	function userRegister() {
		...
		$.ajax({
			...
			url : "./register?usrename="+usrename+"&passwd="+passwd +"&telephone="+telephone+"&email="+email,//要访问的后台地址  
			contentType : "application/json;charset=utf-8",
			data : "", //要发送的数据  
			cache : 'false', 
			beforeSend : function() { //数据发送前报告
...
<body>
	...
	<iframe id="buttom" src="./bottom.jsp"	></iframe>
